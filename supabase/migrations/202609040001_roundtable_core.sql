...
create table public.interview_definitions (
  ...
  duration_minutes integer not null default 30 check (duration_minutes between 2 and 90),
...
create table public.interview_sessions (
  ...
  interview_version_id uuid not null references public.interview_versions(id) on delete restrict,
  organization_id uuid not null references public.organizations(id) on delete cascade,
  status text not null check (status in ('draft','ready','starting','in_progress','assessing','completed','abandoned','failed','needs_review')),
  ...
  phase text not null default 'introduction' check (phase in ('introduction','background','panel','wrap_up')),
